  ...
  <div class="card-body">
    <table class="table">
      ...
      <tr>
        ...
        <th class="text-right item-amount">Price</th>
      </tr>
      </thead>
      @foreach($order->items as $index => $item)
        <tr>
          ...
          <td class="sku-price text-center vertical-middle">RM {{ $item->price }}</td>
          <td class="sku-amount text-center vertical-middle">{{ $item->amount }}</td>
          <td class="item-amount text-right vertical-middle">RM {{ number_format($item->price * $item->amount, 2, '.', '') }}</td>
        </tr>
      @endforeach
    ...
    <div class="order-bottom">
      <div class="order-info">
        ...
        @if($order->ship_data)
          <div class="line">
            <div class="line-label">Express Info：</div>
      ...
      <div class="order-summary text-right">
        ...
        <div class="total-amount">
          ...
          <div class="value">RM {{ $order->total_amount }}</div>
        </div>
        <div>
          <span>PO Status：</span>
          <div class="value">
          ...
          @if(isset($order->extra['refund_disagree_reason']))
            <div>
              <span>Disagree refund reason：</span>
        ...
        @if(!$order->paid_at && !$order->closed)
          <div class="payment-buttons">
            ...
            <!-- 把之前的微信支付按钮换成这个 -->
            <button class="btn btn-sm btn-success" id='btn-wechat'>WePay</button>
          </div>
        @endif
        <!-- 支付按钮结束 -->
        ...
        @if($order->paid_at && $order->refund_status === \App\Models\Order::REFUND_STATUS_PENDING)
          <div class="refund-button">
            <button class="btn btn-sm btn-danger" id="btn-apply-refund">Request Refund</button>
          </div>
        @endif
      </div>
    </div>
    ...
    $(document).ready(function() {
      ...
      $('#btn-wechat').click(function() {
        swal({
          ...
          buttons: ['Close', 'Payment Complete'],
        })

  ...
  <div class="card-body">
          ...
          <td class="product_info">
            ...
            <div @if(!$item->productSku->product->on_sale) class="not_on_sale" @endif>
              ...
              @if(!$item->productSku->product->on_sale)
                <span class="warning">Product is unavailable</span>
              @endif
            </div>
          </td>
          <td><span class="price">RM {{ $item->productSku->price }}</span></td>
          <td>
    ...
    <div>
      <form class="form-horizontal" role="form" id="order-form">
          ...
          <div class="col-sm-9 col-md-7">
            <select class="form-control" name="address">
              @foreach($addresses as $address)
                <option value="{{ $address->id }}"> {{ $address->contact_name }} {{ $address->contact_phone }} {{ $address->full_address }} </option>
              @endforeach
            </select>
          </div>
        </div>
        <div class="form-group row">
          <label class="col-form-label col-sm-3 text-md-right">Memo</label>
          <div class="col-sm-9 col-md-7">
        ...
        <!-- 优惠码开始 -->
        <div class="form-group row">
          <label class="col-form-label col-sm-3 text-md-right">Promo Code</label>
          <div class="col-sm-4">
  ...
  <script>
    $(document).ready(function () {
      ...
      $('.btn-remove').click(function () {
        ...
        swal({
          ...
          .then(function(willDelete) {
            // 用户点击 Confirm 按钮，willDelete 的值就会是 true，否则为 false
            if (!willDelete) {
              return;
            }
      ...
      $('#btn-check-coupon').click(function () {
        ...
        axios.get('/coupon_codes/' + encodeURIComponent(code))
          .then(function (response) {  // then 方法的第一个参数是回调，请求成功时会被调用
            ...
            $('#btn-cancel-coupon').show(); // 显示 Cancel 按钮
            $('#btn-check-coupon').hide(); // 隐藏 检查 按钮
          }, function (error) {
            // 如果返回码是 404，说明优惠券不存在
            if(error.response.status === 404) {
              swal('Invalid Promo Codee', '', 'error');
      ...
      $('#btn-cancel-coupon').click(function () {
        ...
        $('#btn-cancel-coupon').hide(); // 隐藏 Cancel 按钮
